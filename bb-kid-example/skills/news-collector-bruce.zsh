...
show_bruce_reminder_after_news() {
    ...
    if is_workday; then
        ...
        case ${hour} in
            6|7|8)
                echo "  清晨时光，身体准备好了吗？动起来！"
                echo "  💪 年轻也注意保持运动。功夫不是吹出来的。"
                ;;
            9|10|11)
        ...
        case ${hour} in
            6|7|8)
                echo "  周末早晨，适合运动。功夫不是吹出来的。"
                ;;
            9|10|11)
                echo "  周末时光，休息是为了更好地前进。"
                ;;
            *)
                echo "  像水一样，保持流动和活力。"
                ;;
...
collect_news() {
    ...
    if [[ -f "${SCRIPT_DIR}/news-fetcher.sh" ]]; then
        ...
        echo -e "${COLOR_CYAN}🌐 正在连接实时新闻源...${COLOR_NC}"
        echo ""

        # 检查依赖工具
        local dependencies_met=true
        if ! command -v curl >/dev/null 2>&1; then
            echo -e "${COLOR_YELLOW}⚠️  curl 未安装，使用内置新闻源${COLOR_NC}"
            dependencies_met=false
        fi

        if ! command -v jq >/dev/null 2>&1; then
            echo -e "${COLOR_YELLOW}⚠️  jq 未安装，JSON解析功能受限${COLOR_NC}"
        fi

        if [[ "${dependencies_met}" == "true" ]]; then
            if smart_fetch_news "${category}" "${limit}"; then
                # 新闻收集成功后，提供李小龙风格的健康提醒
                show_bruce_reminder_after_news
                return 0
            else
                echo -e "${COLOR_YELLOW}⚠️  实时新闻源获取失败，使用内置新闻源${COLOR_NC}"
                echo ""
            fi
        fi
    else
        echo -e "${COLOR_YELLOW}⚠️  新闻获取模块未找到，使用内置新闻源${COLOR_NC}"
        echo ""
    fi
